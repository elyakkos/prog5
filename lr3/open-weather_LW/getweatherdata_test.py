...
def test_in_riga():
    data = json.loads(get_weather_data("Prague", api_key=key))
    assert data is not None

def test_type_of_res():
    data = get_weather_data("Prague", api_key=key)
...
exp_params_countries = [("New York", key, 'US'),
...
exp_params_timezones = [("New York", key, 'UTC-5'),
                        ("Saint Petersburg", key, 'UTC+3'),
                        ("Severodvinsk", key, 'UTC+6')]
